...
G4VPhysicalVolume* DetectorConstruction::Construct() {
  ...
  G4Box* solidWorld = new G4Box("World",  // its name
                                0.5 * worldWidth, 0.5 * worldHeight,
  ...
  G4ThreeVector globalCaloCenter((nCols % 2 - 1) * crystalWidth / 2.0,
                                 -1 * (nRows % 2 - 1) * crystalHeight / 2.0, 0);

  G4Box* caloPlusLateral =
      new G4Box("caloPlusLateral", 0.5 * crystalWidth * nCols + 0.5,
                0.5 * crystalHeight * nRows + 0.5, 0.5 * crystalLength);
  G4Box* fullCaloSize =
      new G4Box("fullCalo", 0.5 * crystalWidth * nCols,
                0.5 * crystalHeight * nRows, 0.5 * crystalLength);
  G4SubtractionSolid* lateralLeakageSolid =
      new G4SubtractionSolid("lateral", caloPlusLateral, fullCaloSize);

  G4LogicalVolume* logicLateralLeakage =
      new G4LogicalVolume(lateralLeakageSolid, worldMat, "lateral");
  new G4PVPlacement(0, globalCaloCenter, logicLateralLeakage, "lateral",
  ...
  G4Box* albedoLongSolid =
      new G4Box("albedoLong", 0.5 * crystalWidth * nCols + 0.5,
                0.5 * crystalHeight * nRows + 0.5, 0.5);
  G4LogicalVolume* logicAlbedoLong =
      new G4LogicalVolume(albedoLongSolid, worldMat, "albedoLong");
  new G4PVPlacement(
      0, globalCaloCenter - G4ThreeVector(0, 0, 0.5 * crystalLength + 0.5),
      logicAlbedoLong, "albedo", logicWorld, false, 0, checkOverlaps);
  new G4PVPlacement(
      0, globalCaloCenter + G4ThreeVector(0, 0, 0.5 * crystalLength + 0.5),
      logicAlbedoLong, "longitudinal", logicWorld, false, 0, checkOverlaps);
  ...
  // build the crystals
  G4Box* solidCrystal = new G4Box("solidCrystal", 0.5 * crystalWidth,
  ...
  for (int row = 0; row < nRows; ++row) {
    for (int col = 0; col < nCols; ++col) {
      ...
      new G4PVPlacement(0, G4ThreeVector(xPos, yPos, 0), logicalCrystal,
                        "crystal", logicWorld, false, nCols * row + col,
                        checkOverlaps);
    }
  }
  ...
  if (simConf_->wall.on) {
    G4Box* solidWall =
        new G4Box("solidWall", 0.5 * worldWidth, 0.5 * worldHeight,
                  0.5 * simConf_->wall.thickness);

    G4Material* Al = nist->FindOrBuildMaterial("G4_Al");
    G4LogicalVolume* logicalWall =
        new G4LogicalVolume(solidWall, Al, "logicalWall");
    new G4PVPlacement(0, G4ThreeVector(0, 0, -1 * (0.5 * crystalLength +
                                                   simConf_->wall.distance)),
                      logicalWall, "wall", logicWorld, false, 0, checkOverlaps);
