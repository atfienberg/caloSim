...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent) {
  ...
  if (simConf_->wall.on) {
    startingZ =
        -(0.5 * simConf_->calo.length +
          1.25 * (simConf_->wall.distance + 0.5 * simConf_->wall.thickness));
  } else {
  ...
  for (const auto& conf : simConf_->genVector) {
    ...
    if (!conf.randomize) {  // no randomization
      ...
      energy = conf.energy;
             ...
             (G4UniformRand() <
              0.5)) {  // if not first particle, randomize whether we include it
      continue;
    }
    ...
    if (conf.impactAngle == 0) {
      ...
      particleGun_->SetParticleMomentumDirection(G4ThreeVector(
          std::sin(conf.impactAngle * deg) * std::cos(conf.impactPhi * deg),
          std::sin(conf.impactAngle * deg) * std::sin(conf.impactPhi * deg),
          std::cos(conf.impactAngle * deg)));

      G4ThreeVector startingPositionCorrection(
          std::tan(conf.impactAngle * deg) * std::cos(conf.impactPhi * deg),
          std::tan(conf.impactAngle * deg) * std::sin(conf.impactPhi * deg), 0);

      startingPosition = startingPosition +
                         (startingZ + 0.5 * simConf_->calo.length) *
                             startingPositionCorrection;
      particleGun_->SetParticlePosition(startingPosition);
    }
